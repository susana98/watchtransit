...
;hourangle2
        ...
        minhou[l]=min(table[4:6,0,l]+table[4:6,1,l]/60.)
        maxhou[l]=max(table[4:6,0,l]+table[4:6,1,l]/60.)
        midhou[l]=table[4,0,l]+table[4,1,l]/60.
...
if (teles eq 3) then begin 
 ...
 if teles eq 5 then begin
        index=where( (maxalt gt 15) and ( ( minhou lt 2.0) or ( maxhou ge 19.0)),number);;; careful with this is almost only full transits!
    endif else begin
    if teles eq 6 then begin
        index=where( (maxalt gt 25) and ( ( minhou gt 4) and ( maxhou le 16.5)),number);;; careful with this is almost only full transits!
    endif else begin
    if teles eq 7 then begin
        iqndex=where( (maxalt gt 25) and ( ( minhou gt 9) and ( maxhou le 20.)),number);;; careful with this is almost only full transits!  
    endif else begin
        if (teles eq 4) then  begin
       ;;     index=where( (maxalt gt 25) and ( ( minhou gt 0.) and  ( maxhou le 9.0)),number) 
              index=where( (maxalt gt 25) and( ( minhou le 9) or( maxhou ge 24.0)),number)
        endif else  index=where( (maxalt gt 25) and( ( minhou le 6.) or( maxhou ge 20.)),number)
    endelse
endelse
 endelse
endelse
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;full transits only work for the LT!!!!!!!!!!!!
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
index=where( (minalt gt 25) and( ( midhou le 6) or ( midhou ge 19.5)),number)


order=sort(table[0,1,index])
junk=index[order]
openw,unit,fulltransits,/get_lun
printf,unit,'#   Object      DATE        T0    Observing Period  Alt_beg    Alt_To   Alt_end ' 
for i=0, number-1 do begin
    printf, unit,format='(A10,I6.2,"/",I2.2,"/",I4,I5.2,":",I2.2,I6.2,":",I2.2,I4.2,":",I2.2,F10.2,F10.2,F10.2)', namelist[junk[i]],table[3,1,junk[i]], table[3,0,junk[i]], table[3,2,junk[i]],table[4,0,junk[i]], table[4,1,junk[i]],table[5,0,junk[i]], table[5,1,junk[i]],table[6,0,junk[i]], table[6,1,junk[i]], table[1,0,junk[i]], table[1,1,junk[i]], table[1,2,junk[i]]
endfor
...
openw,unit, fulltransitshour ,/get_lun
